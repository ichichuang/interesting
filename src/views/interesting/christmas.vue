...
function createSnowflakeTexture(size: number) {
  ...
  const cx = r
  const cy = r
  g.lineStyle({ width: Math.max(1, size * 0.06), color: 0xffffff, alpha: 0.95, cap: 'round' })
  for (let i = 0; i < 6; i++) {
...
function buildScene() {
  ...
  foliage.lineStyle({ width: 2, color: 0x0f6a3a, alpha: 0.12 })
  ...
  ribbonGlow.lineStyle({ width: 10, color: 0xffd166, alpha: 0.16 })
  const ribbon = new Graphics()
  ribbon.lineStyle({ width: 3, color: 0xffd166, alpha: 0.55 })
